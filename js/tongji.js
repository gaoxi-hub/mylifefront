...
function ar_main() {
	...
	//收集完日志 提交到的路径
	var dest_path   = "http://www.gaoxi.com/admins/recordWebFlow?";

	var expire_time = 30 * 60 * 1000;//会话超时时长

	//标识一个浏览器
	//当用户第一次使用浏览器访问时,为其发一个标识。之后就不再颁发，因为已经有过
	// 独立用户id
	...
	// urlname : a.html
	// title:
	//stat_uv: 是不是新的浏览器（用户） uv_id
	//stat_ss: ss_id；当前会话标识  ss_no 会话次数本次访问跳转或刷新页面时+1 + 浏览器系统now time
